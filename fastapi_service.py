...
# 导入RapidDoc相关模块
from rapid_doc.backend.pipeline.model_init import (
    MineruPipelineModel,
    AtomModelSingleton,
)
...
class ModelConfig(BaseModel):
    ...
    # Formula配置
    formula_engine_type: str = "TORCH".lower()
    formula_level: int = 0
    ...
    table_model_type: str = "UNET_SLANET_PLUS".lower()
    ...
    table_engine_type: str = "ONNXRUNTIME".lower()
...
class ModelManager:
    ...
    def init_models(self, model_config: ModelConfig):
        ...
        try:
            ...
            checkbox_enable = getattr(
                model_config.checkbox_enable, "checkbox_enable", False
            )
            checkbox_config = {
            ...
            self.models["pipeline_model"] = MineruPipelineModel(
                ...
                lang="ch",
                device=os.getenv("MINERU_DEVICE_MODE", "cuda"),
            )
            ...
            logger.info(
                f"All models initialized successfully in {init_time:.2f} seconds"
            )
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.warning(
            "Service will start without pre-initialized models. Models will be loaded on first request."
        )
...
async def parse_document(
    ...
    Args:
        ...
        output_dir: 输出目录
    """
    request = ParseRequest(**json.loads(request))
    # 检查模型是否已初始化
    ...
    try:
        ...
        for file in files:
            ...
            if file_suffix not in [
                ...
                raise HTTPException(
                    status_code=400, detail=f"Unsupported file type: {file_suffix}"
                )
...
async def process_document_async(
    task_id: str, files: List[UploadFile], request: ParseRequest, output_dir: str
):
    """异步处理文档的后台任务"""
    ...
    try:
        ...
        # 这里可以实现具体的异步处理逻辑
        # 目前使用同步处理作为示例
        await parse_document(files=files, request=request, output_dir=output_dir)
